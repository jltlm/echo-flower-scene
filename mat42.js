...
export function lookAt(eye, target, up, dst) {
    ...
    vec3.norm(vec3.sub(eye, target, zAxis), zAxis);
    vec3.norm(vec3.cross(up, zAxis, xAxis), xAxis);
    vec3.norm(vec3.cross(zAxis, xAxis, yAxis), yAxis);
    dst[0] = xAxis[0];
    dst[1] = yAxis[0];
    dst[2] = zAxis[0];
    dst[3] = 0;
